...
export interface ExportToolbarProps extends React.HTMLAttributes<HTMLDivElement> {
...
export const ExportToolbar = React.forwardRef<HTMLDivElement, ExportToolbarProps>(
  function ExportToolbar(
    { formats = ["html", "json", "csv", "pdf"], onExport, onCopyLink, onPrint, busyFormat, className, ...rest },
    ref,
  ) {
    return (
      <div ref={ref} role="toolbar" className={cx("msr-ExportToolbar", className)} {...rest}>
        <span className="msr-ExportToolbar__label">
          <Icon name="export" size={15} /> Export
        </span>
        <div className="msr-ExportToolbar__group">
          {formats.map((f) => (
            <button
              key={f}
              type="button"
              className="msr-ExportToolbar__btn"
        ...
        <span className="msr-ExportToolbar__divider" />
        {onCopyLink && (
          <button type="button" className="msr-ExportToolbar__btn" onClick={onCopyLink}>
            <Icon name="link" size={14} /> Copy link
          </button>
        )}
        {onPrint && (
          <button type="button" className="msr-ExportToolbar__btn" onClick={onPrint}>
