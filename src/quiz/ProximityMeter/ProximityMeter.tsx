...
export interface ProximityMeterProps extends React.HTMLAttributes<HTMLDivElement> {
...
export const ProximityMeter = React.forwardRef<
  HTMLDivElement,
  ProximityMeterProps
>(function ProximityMeter(
  ...
  return (
    <div ref={ref} className={cx("msr-ProximityMeter", className)} {...rest}>
      {showLabel && (
        <div className="msr-ProximityMeter__head">
          <span className="msr-ProximityMeter__label">{proximityLabel(proximity)}</span>
          <span className="msr-ProximityMeter__distance">
            {Math.round(distance).toLocaleString()} {unit}
          </span>
        </div>
      )}
      <div
        className="msr-ProximityMeter__track"
        role="meter"
        aria-valuenow={pct}
        aria-valuemin={0}
        aria-valuemax={100}
      ...
      >
        <span
          className="msr-ProximityMeter__marker"
          style={{ left: `${pct}%` }}
          data-proximity={pct >= 80 ? "hot" : pct >= 40 ? "warm" : "cold"}
        />
      </div>
      <div className="msr-ProximityMeter__scale">
        <span>Far</span>
        <span>Close</span>
      </div>
    </div>
